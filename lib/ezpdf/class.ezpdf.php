...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  foreach($cols as $colName=>$colTitle){
    ...
    ### monte hacks to get colwidth to work as option[option]
		if(isset($options['options'][$colName]["width"]))
		{
			$maxWidth[$colName]=$options['options'][$colName]["width"];
		}
		// end hack
  }
  ...
  if ($options['width'] && $adjustmentWidth>0 && $setWidth<$options['width']) {
    ...
    //echo 'neededWidth: ' . $neededWidth . '<Br>';
    //echo 'presentWdith: ' . $presentWidth . '<br>';
